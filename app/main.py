...
def quarantine_list(
    ...
    action_state: str = "all",
    learning_mark: str = "all",
    page: int = Query(1, ge=1),
    ...
    action_state = str(action_state or "all").strip().lower()
    if action_state not in {"all", "pending", "release_ham", "mark_spam"}:
        raise HTTPException(status_code=400, detail="Invalid Actions filter")
    learning_mark = str(learning_mark or "all").strip().lower()
    if learning_mark not in {"all", "unmarked", "ham", "spam"}:
        raise HTTPException(status_code=400, detail="Invalid Learning Mark filter")
    try:
...
<option value="does_not_contain">Does not contain</option>
  ...
  <div class="filter-group"><label>Actions</label><select id="qActionState" onchange="setQuarantineActionState(this.value)"><option value="all">All</option><option value="pending">Pending Action</option><option value="release_ham">Release + HAM</option><option value="mark_spam">Mark Spam</option></select></div>
  <div class="filter-group"><label>Learning Mark</label><select id="qLearningMark" onchange="setQuarantineLearningMark(this.value)"><option value="all">All</option><option value="unmarked">Unmarked HAM/SPAM</option><option value="ham">Marked HAM</option><option value="spam">Marked SPAM</option></select></div>
  <div class="filter-group"><label>Admin Review Queue</label><select id="qAdminDecision" onchange="setQuarantineAdminDecision(this.value)" title="Administrator Ground Truth review state"><option value="all">All</option><option value="required">Required</option><option value="completed">Completed</option></select></div>
...
<div id="helpTab" class="tabpane">
  ...
  <div class="help-grid">
    ...
    <section class="help-card"><h3>R1.1.57 Admin Review Queue Filter</h3><p>Quarantine places <b>Admin Review Queue</b> as a compact LOV below the Learning Mark filter, with All / Required / Completed states. <b>Required</b> means the quarantine item has no authoritative CURRENT administrator Ground Truth row. Use the existing <b>Intelligence</b> button to inspect a message; opening Intelligence never creates Ground Truth. Only an explicit administrator save/acknowledgement creates the Set-2 Ground Truth record. Historical or superseded rows do not satisfy the requirement. The previous summary card, pending-review button, and duplicate per-row Review Now control are removed.</p></section>
    <section class="help-card"><h3>R1.1.56 Quarantine Action & Learning LOV Filters</h3><p>Quarantine adds two compact persisted dropdown LOV filters. <b>Actions</b> provides All / Pending Action / Release + HAM / Mark Spam. Pending Action means neither the completed Release + HAM state nor Mark Spam has been recorded. <b>Learning Mark</b> provides All / Unmarked HAM/SPAM / Marked HAM / Marked SPAM and is driven by the persisted dashboard sa-learn compatibility flags. The filters can be combined with Admin Decision; for example, Admin Decision = Required + Actions = Pending Action + Learning Mark = Unmarked HAM/SPAM identifies messages with no authoritative AI Ground Truth, no finalized quarantine action, and no HAM/SPAM learning mark. Quarantine actions and SpamAssassin learning remain independent from AI Set-2 Ground Truth.</p></section>
...
function saveUxFilters(){
  const ids=["searchField","searchOperator","search","dateFrom","dateTo","filter","deliveryPageSize","qSearchField","qSearchOperator","qSearch","qDate","qAdminDecision","qActionState","qLearningMark","qPageSize","slSearch","slPreference","slScope","slPageSize","auditSearchOperator","auditSearch","auditDateFrom","auditDateTo","auditAction","auditPageSize"];
...
function restoreUxFilters(){
  ...
  quarantineActionState=["pending","release_ham","mark_spam"].includes(String(document.getElementById("qActionState")?.value||"").toLowerCase())?String(document.getElementById("qActionState").value).toLowerCase():"all";
  quarantineLearningMark=["unmarked","ham","spam"].includes(String(document.getElementById("qLearningMark")?.value||"").toLowerCase())?String(document.getElementById("qLearningMark").value).toLowerCase():"all";
...
function clearQuarantineFilters(){document.getElementById("qSearchField").value="all";document.getElementById("qSearchOperator").value="contains";document.getElementById("qSearch").value="";document.getElementById("qDate").value="";quarantineCategory="all";quarantineAdminDecision="all";quarantineActionState="all";quarantineLearningMark="all";const qAdminDecision=document.getElementById("qAdminDecision");if(qAdminDecision)qAdminDecision.value="all";const qActionState=document.getElementById("qActionState");if(qActionState)qActionState.value="all";const qLearningMark=document.getElementById("qLearningMark");if(qLearningMark)qLearningMark.value="all";document.querySelectorAll(".qmetric-filter").forEach(b=>b.classList.toggle("active",b.dataset.qfilter==="all"));qp=1;updateQuarantineUx();loadQuarantine(1);}
...
let quarantineActionState="all";
let quarantineLearningMark="all";
...
function setQuarantineActionState(value){
  quarantineActionState=["pending","release_ham","mark_spam"].includes(String(value||"").toLowerCase())?String(value).toLowerCase():"all";
  const el=document.getElementById("qActionState"); if(el) el.value=quarantineActionState;
  loadQuarantine(1);
}
function setQuarantineLearningMark(value){
  quarantineLearningMark=["unmarked","ham","spam"].includes(String(value||"").toLowerCase())?String(value).toLowerCase():"all";
  const el=document.getElementById("qLearningMark"); if(el) el.value=quarantineLearningMark;
  loadQuarantine(1);
}
...
async function loadQuarantine(pageNumber=qp,options={}){
  ...
  const query=new URLSearchParams({
    ...
    action_state:quarantineActionState,
    learning_mark:quarantineLearningMark,
    page:qp,
  ...
  const actionFilter=document.getElementById("qActionState"); if(actionFilter) actionFilter.value=quarantineActionState;
  const learningFilter=document.getElementById("qLearningMark"); if(learningFilter) learningFilter.value=quarantineLearningMark;
