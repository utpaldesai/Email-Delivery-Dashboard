...
def query_items(q="", q_field="all", q_operator="contains", date="", category="all", page=1, page_size=None, admin_decision="all", action_state="all", learning_mark="all", decided_pdp_ids=None):
    q = (q or "").strip()
    ...
    action_state = (action_state or "all").strip().lower()
    if action_state not in {"all", "pending", "release_ham", "mark_spam"}:
        raise ValueError("Invalid Actions filter")
    learning_mark = (learning_mark or "all").strip().lower()
    if learning_mark not in {"all", "unmarked", "ham", "spam"}:
        raise ValueError("Invalid Learning Mark filter")
    ...
    for item in source:
        ...
        if admin_decision == "completed" and not has_admin_decision:
            continue
        learning = str(item.get("learning") or "").strip().lower()
        release_ham_complete = bool(item.get("is_released")) and learning == "ham"
        mark_spam_complete = bool(item.get("is_manual_spam"))
        if action_state == "pending" and (release_ham_complete or mark_spam_complete):
            continue
        if action_state == "release_ham" and not release_ham_complete:
            continue
        if action_state == "mark_spam" and not mark_spam_complete:
            continue
        if learning_mark == "unmarked" and learning in {"ham", "spam"}:
            continue
        if learning_mark == "ham" and learning != "ham":
            continue
        if learning_mark == "spam" and learning != "spam":
            continue
        row = dict(item)
        ...
        row["action_state"] = "RELEASE_HAM" if release_ham_complete else ("MARK_SPAM" if mark_spam_complete else "PENDING")
        row["learning_mark"] = learning.upper() if learning else "UNMARKED"
        filtered.append(row)
    ...
    return {
        ...
        "action_filter": action_state,
        "learning_filter": learning_mark,
    }
