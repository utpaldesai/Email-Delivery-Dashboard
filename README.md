# Postfix Delivery Dashboard — R1.1.57 Quarantine Admin Review LOV Realignment + Action/Learning Filters

## R1.1.56 changes

- Adds compact persisted quarantine dropdown LOV filters for **Actions** and **Learning Mark**.
- Actions: All / Pending Action / Release + HAM / Mark Spam. Pending Action means neither completed Release + HAM nor Mark Spam is recorded.
- Learning Mark: All / Unmarked HAM/SPAM / Marked HAM / Marked SPAM, based on persisted dashboard learning flags.
- Filters combine with Admin Decision so administrators can isolate messages still missing both operational action and HAM/SPAM learning.
- Quarantine actions and SpamAssassin learning remain independent from AI Set-2 Ground Truth.
- No Postfix/Amavis mail-flow change and no MariaDB schema migration or destructive database operation.

## R1.1.57 changes

- Removes the separate Admin Review Queue summary card and Review Pending Messages button.
- Removes any duplicate per-row Admin Decision Required / Review Now control; the existing Intelligence button remains the single message-review entry point.
- Places **Admin Review Queue** as its own compact LOV directly below **Learning Mark**. Values remain All / Required / Completed.
- Keeps **Learning Mark** separate from Admin Review Queue: All / Unmarked HAM/SPAM / Marked HAM / Marked SPAM.
- Keeps **Actions** separate: All / Pending Action / Release + HAM / Mark Spam.
- Admin Review Queue continues to use authoritative CURRENT Admin Ground Truth state; opening Intelligence never creates Ground Truth.
- No database migration, destructive operation, Postfix/Amavis mail-flow change, or AI behavior change.


# Postfix Delivery Dashboard — R1.1.55 Admin Review Queue De-duplication + Semantic Intelligence

- Candidate fitting and auto-train counts now accept only CURRENT schema-v4 administrator Ground Truth (`admin-ground-truth`, `admin-ground-truth-ui`, `admin-ai-acknowledged`).
- Historical `sa-learn`, Bayes backfill, learning-correction, missing-source and legacy-generation rows are preserved for audit but excluded from AI training eligibility. No existing MariaDB or immutable trainer history is deleted or rewritten.
- The legacy AI label backfill endpoint is retained only as a blocked/audited compatibility endpoint; it no longer imports SpamAssassin/Bayes state into Set-2.
- Learn Spam / Learn Ham and correction actions continue to update production SpamAssassin/Bayes exactly as before, but no longer mirror those outcomes into the independent AI trainer.
- Trainer status exposes a provenance audit with eligible versus excluded rows and exclusion counts by label source.
- Infrastructure AI now reports provider-aware Google/Microsoft hop context and provider transitions as contextual evidence only. Google Workspace, Microsoft 365, forwarding and hybrid routes are explicit exceptions; provider mismatch never by itself means SPAM and authentication PASS never implies HAM.
- Feature schema remains v4, seven feature families remain unchanged, Hard-HAM weight remains 1.75, and AI remains SHADOW ONLY.
- No destructive database migration is introduced. Stable application image remains `postfix-delivery-dashboard:m7`.

# Postfix Delivery Dashboard — R1.1.48 Ground-Truth Persistence + Recursive Attachment Intelligence

- Quarantine Intelligence now restores the authoritative CURRENT Mail Admin Ground Truth from MariaDB whenever the same quarantined message is reopened. Saved administrator HAM/SPAM classification takes precedence over the AI proposal in the editable controls.
- Added a visible Saved Admin Ground Truth status with reviewer/time context. Existing immutable history and calibration tables are reused; no destructive database migration is introduced.
- Independent Message AI now performs bounded, local-only recursive ZIP member inspection inside the existing `mime_attachment` feature family. Nested ZIP depth, embedded dangerous script/executable extensions, and nested-archive indicators are available as SHADOW evidence and hashed model features.
- Archive members are never executed or written to disk. Scan depth/member/expanded-byte limits protect against archive bombs. No Amavis/SpamAssassin verdict, score, or rule is used as an AI feature.
- Added regression coverage for ZIP → ZIP → `.bat` detection and ground-truth reload wiring.
...
The former **Sync Existing Labels** path is retained only as a compatibility/audit endpoint and is blocked from importing SpamAssassin/Bayes decisions into the AI dataset. Historical rows remain preserved for audit. Candidate training and auto-train counts include only current schema-v4 administrator Ground Truth sources.
...
- Removes the duplicate per-record Admin Decision Required / Review Now control from the quarantine row because the existing Intelligence button opens the same Quarantine Intelligence review screen.
